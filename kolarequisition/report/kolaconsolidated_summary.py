...
class KolaConsolidatedSummaryReport(models.AbstractModel):
	_name = 'report.consolidated.report_consolidate_purchase_request'
	...
	def _get_purchase_requests(self):
		res = []
		for request in self.env['kola.requisition'].search([]):
			res.append({'name': request.name, 'state': request.state})
		return res
	...
	def _get_report_values(self, docids, data=None):
		...
		return {
			...
			'get_purchase_requests': self._get_purchase_requests()
		}
